...
- 主机配置存 `%LOCALAPPDATA%/AdvancedRdp/hosts.json`。
- 密码存 Windows Credential Manager（Generic Credentials，键名：`AdvancedRdp/advancedrdp:{名称}`）。
...
2) 命令行构建/运行：
   ...
   ```
3) 首次运行时，添加主机并勾选“保存密码”，密码会写入 Credential Manager；以后可空密码直接连接。

## 已知事项与下一步
- UI 是 MVP：未做尺寸设置、标签页和重连策略，可后续补充。
- 若系统禁用 ActiveX 或移除 MsTscAx，需启用“远程桌面连接”组件。
- 可追加：UDP/GFX/H.264 细化开关、多窗口/多标签、导入导出配置、自更新。 
